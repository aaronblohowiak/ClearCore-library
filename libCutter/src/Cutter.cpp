...
void Controller::DispatchCommand(const ParsedCommand& cmd) {
    ...
    if (strcmp(cmd.name, "get_pins") == 0) {
        CmdGetPins(cmd);
        return;
    }
    if (strcmp(cmd.name, "get_motors") == 0) {
        CmdGetMotors(cmd);
        return;
    }
...
void Controller::CmdGetPins(const ParsedCommand& cmd) {
    ...
    if (configured_count == 0) {
        ...
        m_response.Param("count", static_cast<int32_t>(0));
        SendResponse();
        return;
    }
...
void Controller::CmdGetMotors(const ParsedCommand& cmd) {
